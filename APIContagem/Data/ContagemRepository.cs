...
public class ContagemRepository
{
    ...
    public ContagemRepository(ILogger<ContagemRepository> logger,
    ...
    public void Save(ResultadoContador resultado)
    {
        try
        {
            ...
            _logger.LogInformation(
                $"MongoDB - Gerado documento para o valor {resultado.ValorAtual} | " +
                $"{resultadoJson}");
        }
        catch (Exception ex)
        {
            this._logger.LogError(ex, "Erro na gravação do documento.");
            throw;
        }
    }
}